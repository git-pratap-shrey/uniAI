...
**Functions:**
- `infer_metadata_from_path(pdf_path) -> dict` -- Parses flattened path `<SUBJECT>/notes/unit<N>/*.pdf` to get subject, type, unit. Normalizes unit to numeric string. Returns a dict containing `subject`, `type`, `unit`, and `source_pdf`.
...
- `process_pdf(pdf_path) -> None` -- Main per-PDF logic:
  ...
  4. If page is new: renders image (JPEG for Ollama, PNG bytes for HF), calls `notes_extraction(existing_topics)` prompt via `models.vision()`, retries 3× with 5s×attempt backoff on failure (including `⚠ Vision Error` returns).
...
- `load_pdf(pdf_path) -> str` -- Renders each page, calls VLM with `PYQ_VLM_TRANSCRIPTION` prompt per page via `models.vision()`. Formats images as JPEG for Ollama and raw PIL Images for HF. Scale=1.0 for Ollama (to avoid Cloudflare timeouts), 1.5 for HF. Retries 3× with **15s×attempt** backoff on failure.
...
- `get_unit_classification(question_text, syllabus_text) -> int` -- Calls `models.chat()` with `pyq_unit_classification()` prompt, using the `CONFIG["model"]["model"]` and `CONFIG["providers"]["chat"]` configuration. Retries 3× with 15s×attempt backoff. Returns unit 1-5, default 1 on failure.
...
- `call_vlm(images, max_retries=3) -> dict|None` -- Calls `models.vision()` with `SYLLABUS_EXTRACTION` prompt, parses JSON via `extract_first_json()`. Retries 3× with **15s×attempt** backoff on failure.
...
- `process_syllabus(pdf_path, force=False) -> None` -- Renders pages via `render_pdf_to_images` (scale=1.0 for Ollama, 2.0 for others), calls VLM per page, accumulates data, writes 7 JSON chunk files. Skips if all 7 exist (unless forced).
...
- Syllabus: 15s × attempt (up to 3 attempts)
