...
| `generate_unit_embeddings.py` | `CONFIG["paths"]["unit_embeddings"]` | Hybrid router Stage 2 (embedding similarity) |
...
**`embed(texts: list[str]) -> list[list[float]]`** -- Calls `models.embed()` using the configured embedding provider (defaulting to `ollama`).
...
**Constants:** `STOP_WORDS`, `MAX_ITEMS_PER_UNIT=30`, `MAX_ITEMS_PER_SUBJECT=50`, `MAX_KEYWORD_WORDS=5`, `OUTPUT_FILE`, `CHROMA_PATH`, `MODEL`, `COLLECTIONS`.
...
- `dedupe(lst: list[str]) -> list[str]` -- Deduplicates a list of strings while preserving order.
...
- `main() -> None` -- Builds texts, generates embeddings via `local_embedding.embed()`, saves as pickle to `CONFIG["paths"]["unit_embeddings"]`.

### `retrieval_utils.py`

**`retrieve_with_threshold(collection, query, n_initial=10, similarity_threshold=None, metadata_filter=None) -> dict`** -- Generates query embedding, queries ChromaDB, converts distances to similarity (`1.0 - distance` for cosine space), filters results below threshold (defaults to `CONFIG["rag"]["similarity_threshold"]`). Returns filtered dict matching ChromaDB structure.
...
**Dependencies:**
- `generate_keyword_map.py` reads ChromaDB, uses `prompts.keyword_extraction` (and direct `ollama.Client`), writes `subject_keywords.json`
- `generate_unit_embeddings.py` reads `subject_keywords.json`, uses `embeddings/local_embedding.py` which delegates to `models.embed()`, writes to `CONFIG["paths"]["unit_embeddings"]`
- `retrieval_utils.py` uses `embeddings/local_embedding.py` for query-time retrieval
